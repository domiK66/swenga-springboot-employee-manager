...
interface EmployeeRepository : JpaRepository<Employee, Int> {
    ...
    @Query("SELECT e FROM Employee AS e WHERE LOWER(CONCAT(e.firstName, ' ', e.lastName)) LIKE CONCAT('%', LOWER(COALESCE(:search, '')), '%') AND (:department IS NULL OR e.department = :department) AND :user MEMBER OF e.users ")
    fun findBySearchText(@Param("search") search: String?, @Param("department") department: Department?, @Param("user") user: User): List<Employee>
}
